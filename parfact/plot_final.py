...
ss = json.loads((D / "curves_v2_ssimplex.json").read_text())
...
series = [
    ...
    ("co-fac v2 + U-simplex", "#d4437c", us["curves"]["canonical"],
     us["curves"]["oracle"]),
    ("co-fac v2 + U,S-simplex", "#e0862c", ss["curves"]["canonical"],
     ss["curves"]["oracle"]),
    ("SPD", "#1baf7a", base["spd"]["curves"]["canonical"],
     base["spd"]["curves"]["oracle"]),
]
...
out = Path(__file__).parent / "figures/keep_topk_final.png"
fig.tight_layout()
fig.savefig(out)
print("wrote", out)
