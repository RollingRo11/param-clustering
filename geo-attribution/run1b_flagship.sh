...
mkdir -p "$ROOT"
SENSOR="--sensor ig --ig_k 5 --scalar equal_reward --bf16 --compile --no-fused_attention"
C=8192; N_POS=32000000; PILOT=262144; QUOTA=4
...
CUDA_VISIBLE_DEVICES=0 "$PY" collect_fast.py spec \
  --artifact_root "$ROOT" --tag fl_spec_w2 --feat_dim 65536

step "1 bootstrap pilot"
torchrun --nproc_per_node=2 collect_fast.py collect \
  --artifact_root "$ROOT" --tag fl_pilot_boot --spec_tag fl_spec_w2 \
...
CUDA_VISIBLE_DEVICES=0 "$PY" collect_fast.py fit_stream \
  --artifact_root "$ROOT" --tag fl_stream_boot --pilot_tag fl_pilot_boot \
...
torchrun --nproc_per_node=2 collect_fast.py stream \
  --artifact_root "$ROOT" --tag fl_stream_boot --spec_tag fl_spec_w2 \
  --stream_model_tag fl_stream_boot --data_path "$CORPUS" \
...
CUDA_VISIBLE_DEVICES=0 "$PY" collect_fast.py spec_stats \
  --artifact_root "$ROOT" --tag fl_spec_stats --val_tag fl_stream_boot \
  --feat_dim 65536 --stats_rows 2048

rm -rf "$ROOT/fl_stream_boot" "$ROOT/fl_pilot_boot"
df -h /dev/shm | tail -1
step "5 production pilot"
torchrun --nproc_per_node=2 collect_fast.py collect \
  --artifact_root "$ROOT" --tag fl_pilot --spec_tag fl_spec_stats \
  --data_path "$CORPUS" --master_dtype bfloat16 \
  --n_positions "$PILOT" --pos_per_seq 506 --sub_per_seq 0 \
  --batch_seqs 32 --seq_len 512 --data_order sequential $SENSOR
...
CUDA_VISIBLE_DEVICES=0 "$PY" collect_fast.py fit_stream \
  --artifact_root "$ROOT" --tag fl_streamC8192 --pilot_tag fl_pilot \
  --C "$C" --embed_dim 256 --pilot_max_positions "$PILOT"

step "7 main stream: $N_POS positions"
torchrun --nproc_per_node=2 collect_fast.py stream \
  --artifact_root "$ROOT" --tag fl_streamC8192 --spec_tag fl_spec_stats \
  --stream_model_tag fl_streamC8192 --data_path "$CORPUS" \
...
CUDA_VISIBLE_DEVICES=0 "$PY" geo1b.py extract_ps --tag fl_streamC8192 \
  --C "$C" --soft_T 1.0 --soft_s 8 --banks_tag flagship

echo "FLAGSHIP_DONE: $ROOT/fl_streamC8192"
date -Is
