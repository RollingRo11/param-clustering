...
def pick(obj, B):
    ...
    for a in ("component", "lora"):
        pts = [p for p in rows if p["objective"] == obj and p["budget"] == B
               and p["arm"] == a and p["dev"]["english"] <= EN_BUDGET]
        if not pts:
            return None
        ok = [p for p in pts if p["dev"]["romance"] <= ROM_CAP]
        if ok:
            out[a] = (max(ok, key=lambda p: p["dev"]["german"]), True)
        else:
            out[a] = (min(pts, key=lambda p: p["dev"]["romance"]), False)
    return out["component"], out["lora"]
...
figure("english_only", "out/fair_bars_english_only",
       "Most German removable inside a collateral budget — English protected, Romance undefended",
       "The Romance KL term is removed: nothing defends French, Spanish or Italian. Same rule for both methods over the full sweep of\n"
       "(learning rate, lambda, training step): take the most German removal available while keeping Romance damage under 0.10 nats\n"
       "and English under 0.15. Chosen on dev/guard blocks, reported on held-out blocks.")
print()
figure("multilingual", "out/fair_bars_multilingual",
